            ...
            if INDEX > COUNT:
                ...
                if download.status() != MirrorStatus.STATUS_ARCHIVING and download.status() != MirrorStatus.STATUS_EXTRACTING:
                    ...
                    if download.status() == MirrorStatus.STATUS_CLONING:
                        msg += f"\n<b>☞ 💭 ᴄʟᴏɴᴇᴅ:</b> <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                    ...
                    msg += f"\n<b>☞ ⚡️ ꜱᴘᴇᴇᴅ :</b> <code>{download.speed()}</code>" \
                            f"\n<b>☞ ⏰ ᴇᴛᴀ :</b> <code>{download.eta()}</code> "
                    # if hasattr(download, 'is_torrent'):
                    try:
                        msg += f"\n<b>☞ 🌱 sᴇᴇᴅᴇʀs :</b> <code>{download.aria_download().num_seeders}</code>" \
                            f" | <b>☞ 🍂 ᴘᴇᴇʀs :</b> <code>{download.aria_download().connections}</code>"
                    except:
                        pass
                    try:
                        msg += f"\n<b>☞ 🌱 sᴇᴇᴅᴇʀs :</b> <code>{download.torrent_info().num_seeds}</code>" \
                            f" | <b>☞ 🐞 ʟᴇᴇᴄʜᴇʀs :</b> <code>{download.torrent_info().num_leechs}</code>"
                    except:
                        pass
                    msg += f"\n<b>☞ ⛔️ ᴛᴏ ꜱᴛᴏᴘ:</b> <code>/{BotCommands.CancelMirror} {download.gid()}</code>"
                msg += "\n\n"
                if STATUS_LIMIT is not None:
